...
fn write_rustc_version(path: &PathBuf) {
    ...
    let mut rustc_version_file =
        fs::File::create(path.join("rustc.version")).expect("Couldn't open rustc version file");
    ...
    .expect("Couldn't write rustc version file");
}
...
fn parse_config(log_file: &mut Option<std::fs::File>, text: &str) -> Option<Config> {
    ...
    if id.is_none() {
        log(
            log_file,
            "failed to parse config.txt: missing participant_id",
        );
        None
    } else if pwd.is_none() {
        ...
        Some(Config {
            participant_id: id.unwrap().to_owned(),
            git_password: pwd.unwrap().to_owned(),
            project: proj.unwrap().to_owned(),
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_read_config() {
        let text = "\"participant_id\": \"592089\",
            \"git_password\": \"985613\",
            \"project\":\"p1\"";
